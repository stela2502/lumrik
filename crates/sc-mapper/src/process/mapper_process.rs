...
use crate::process::{
    MapperRecord, SamClusterBuffer, SamClusterReceiver, SamReadCluster, sam_cluster_channel,
};
...
impl MapperProcess {
    ...
    pub fn spawn_single_stdin(
        ...
        let mut child = Command::new(binary)
            ...
            .spawn()
            .with_context(|| format!("failed to spawn mapper `{}`", binary.display(),))?;
        ...
        let stdin = child.stdin.take().context("failed to open mapper stdin")?;

        let (clusters, header_rx, stdout_thread) = spawn_stdout_cluster_thread(stdout);
    ...
    pub fn spawn_single_fifo(
        ...
        let r1 = OpenOptions::new()
            .write(true)
            .open(&r1_path)
            .with_context(|| format!("failed to open FASTQ FIFO `{}`", r1_path.display(),))?;

        Ok(Self {
            child,

            input: Some(FastqInput::SingleFifo { r1 }),

            /*
             * CRITICAL:
             * move both TempDirs into MapperProcess.
             */
            _input_tmpdir: Some(input_tmpdir),

            _work_tmpdir: Some(work_tmpdir),
    ...
    pub fn spawn_paired_fifo(
        ...
        let mut child = Command::new(binary)
            ...
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to spawn paired mapper `{}`", binary.display(),))?;
        ...
        let r1_open = thread::spawn(move || -> Result<File> {
            OpenOptions::new()
                .write(true)
                .open(&r1_open_path)
                .with_context(|| format!("failed to open R1 FIFO `{}`", r1_open_path.display(),))
        });
        ...
        let r2_open = thread::spawn(move || -> Result<File> {
            OpenOptions::new()
                .write(true)
                .open(&r2_open_path)
                .with_context(|| format!("failed to open R2 FIFO `{}`", r2_open_path.display(),))
        });
        ...
        Ok(Self {
            ...
            input: Some(FastqInput::PairedFifo { r1, r2 }),
    ...
    pub fn spawn_paired_fifo_with_arg_insertion(
        ...
        Self::spawn_paired_fifo(binary, base_args, header)
    }
...
impl MapperProcessLike for MapperProcess {
    ...
    fn write_fastq(&mut self, r1: &FastqRecord, r2: Option<&FastqRecord>) -> Result<()> {
        ...
        match input {
            ...
            FastqInput::PairedFifo { r1: w1, r2: w2 } => {
                let Some(r2) = r2 else {
    ...
    fn is_running(&mut self) -> Result<bool> {
        match self
            .child
            .try_wait()
            .context("failed to query mapper process state")?
        {
            None => Ok(true),

            Some(status) => {
                eprintln!("[sc-mapper] mapper exited with status {status}");
    ...
    fn header(&mut self) -> Result<&Header> {
        ...
        Ok(self.header.as_ref().expect("header was initialized above"))
    }
...
fn spawn_stdout_cluster_thread(
...
) -> (SamClusterReceiver, Receiver<Header>, JoinHandle<Result<()>>) {
    let (tx, rx) = sam_cluster_channel(DEFAULT_CLUSTER_CHANNEL_SIZE);
    ...
    let handle = thread::spawn(move || -> Result<()> {
         ...
         */
        let mut reader = bam_reader_from_child_stdout(stdout)
            .context("failed to create BAM reader for mapper stdout")?;
        ...
        header_tx
            .send(header)
            .context("failed to publish mapper BAM header")?;

        let mut buffer =
            SamClusterBuffer::new(tx, DEFAULT_CLUSTER_MAX_GAP, DEFAULT_CLUSTER_FLUSH_EVERY);

        for record in reader.records() {
            let record = record.context("failed to read mapper SAM/BAM record")?;

            buffer.push(MapperRecord::new(record))?;
        }
...
#[cfg(unix)]
fn bam_reader_from_child_stdout(stdout: std::process::ChildStdout) -> Result<bam::Reader> {
    ...
    let reader = bam::Reader::from_path(&path)
        .with_context(|| format!("failed to create BAM reader from mapper stdout fd {fd}"))?;
