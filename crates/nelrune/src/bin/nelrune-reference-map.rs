...
#[command(
    author,
    version,
    about = "Map unresolved reference_curator candidates and attach raw genomic alignment evidence"
)]
struct Args {
    #[arg(long)]
    store: PathBuf,
...
fn main() -> Result<()> {
    ...
    if args.mapper.mapper_paired {
        bail!("reference candidate mapping is single-end; omit --mapper-paired");
    }
    let mut curator = ReferenceCurator::open(&args.store)
        .with_context(|| format!("opening curator {}", args.store.display()))?;
    let mut candidates: Vec<_> = curator
        .unresolved()
        .map(|c| (c.id.clone(), c.sequence.clone()))
        .collect();
    candidates.sort_by(|a, b| a.0.cmp(&b.0));
    if candidates.is_empty() {
        bail!("curator contains no unresolved candidates");
    }

    let bam_path = args
        .bam_out
        .clone()
        .unwrap_or_else(|| args.store.with_extension("mapping.bam"));
    ...
    for (id, seq) in &candidates {
        ...
        if writer.is_none() && mapper.header_loaded() {
            writer = Some(open_writer(&bam_path, &mut mapper)?);
        }
    ...
    let producer = Producer {
        name: format!("{:?}", args.mapper.mapper),
        version: None,
        parameters: Some(format!(
            "index={} threads={} options={}",
            args.mapper.mapper_index.display(),
            args.mapper.mapper_threads,
            args.mapper.mapper_options.as_deref().unwrap_or("")
        )),
    };
    let mut added = 0usize;
    for ev in evidence {
        if curator.candidate(&ev.query).is_none() {
            continue;
        }
        ...
        a.cigar = Some(ev.cigar);
        a.mapq = Some(ev.mapq);
        a.edit_distance = ev.edit_distance;
        a.query_start = ev.query_start;
        a.query_end = ev.query_end;
        a.secondary = ev.secondary;
        a.supplementary = ev.supplementary;
        curator.add_annotation(&ev.query, args.reference_id.clone(), a)?;
        added += 1;
    }
