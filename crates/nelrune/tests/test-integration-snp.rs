...
use snp_index::{SnpIndex, VcfReadOptions};
...
fn integration_tiny_star_snp() {
    ...
    let vcf = test_path("tiny.vcf");

    let r1 = test_path("tiny_R1_snp.fastq");

    let r2 = test_path("tiny_R2_snp.fastq");

    for path in [&fasta, &gtf, &vcf, &r1, &r2] {
        require_file(path);
    }
    ...
    let out = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/test-integration-tiny-snp");
    ...
    let snp_index = SnpIndex::from_vcf_path(
        &vcf,
        vec!["chrTiny".to_string()],
        vec![400u32],
        10_000,
        &VcfReadOptions::default(),
    )
    .expect("failed to load tiny SNP index");

    let tiny_snp = snp_index
        .feature_id("tiny_snp")
        .expect("tiny_snp missing from SNP index");
    ...
    let exonic_cell_ids = exonic_data.cell_ids();

    let cell_id = *exonic_cell_ids.iter().next().unwrap();
    ...
    let ref_data =
        Scdata::read_matrix_market(&snp_ref, &snp_index).expect("failed to reload SNP REF matrix");

    let alt_data =
        Scdata::read_matrix_market(&snp_alt, &snp_index).expect("failed to reload SNP ALT matrix");
    ...
    assert!(
        ref_data.cell_ids().contains(&cell_id),
        "gene and SNP REF outputs should contain the same cell"
    );

    assert!(
        alt_data.cell_ids().contains(&cell_id),
        "gene and SNP ALT outputs should contain the same cell"
    );

    let ref_cell = ref_data
        .get(&cell_id)
        .expect("cell missing from SNP REF Scdata");

    let alt_cell = alt_data
        .get(&cell_id)
        .expect("cell missing from SNP ALT Scdata");

    assert_eq!(
        ref_cell.total_umis_4_gene_id(&tiny_snp),
        2.0,
        "expected exactly two REF-supporting UMIs for chrTiny:135 A>C"
    );

    assert_eq!(
        alt_cell.total_umis_4_gene_id(&tiny_snp),
        2.0,
