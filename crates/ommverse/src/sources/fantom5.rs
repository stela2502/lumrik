...
pub const HG38_ENHANCERS_URL: &str = "https://fantom.gsc.riken.jp/5/datafiles/reprocessed/hg38_latest/extra/enhancer/F5.hg38.enhancers.bed.gz";
...
pub fn fetch(assembly: &str, root: &Path) -> Result<Option<PathBuf>> {
    let Some(url) = enhancer_url(assembly) else {
        return Ok(None);
    };
