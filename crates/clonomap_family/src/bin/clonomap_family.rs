...
fn main() -> Result<()> {
    let args = Args::parse();
    if !(0.0..=1.0).contains(&args.max_pearson_p) { bail!("--max-pearson-p must be between 0 and 1"); }
    ...
    for (cell_id, rows) in by_cell {
        ...
        match hcs.as_slice() {
            ...
            [hc] => {
                let lc = rows.iter()
                    .filter(|r| r.productive && matches!(r.receptor.chain.as_str(), "IGK" | "IGL") && receptor_complete(&r.receptor))
                    .map(|r| r.receptor.clone())
                    .collect();
                qualified.push(CellReceptor { cell_id, hc: hc.receptor.clone(), lc });
            }
            _ => multiple_hc += 1,
        }
    }
    ...
    println!("\nInitial HC families: {} ({provisional_cells} cells)", families.len());
    ...
    for family in &mut families {
        ...
        rejected.extend(family.align(&cfg, &novel_v).into_iter().map(|cell| (from.clone(), cell)));
    }
    ...
    for family in &mut families { family.split_light_chains(&cfg, &novel_v); }

    write_family_report(&args.out.join("families.tsv"), &families)?;
    let cell_id_detectors: Vec<PrimerDetector> = args.chemistry.iter().copied()
    ...
    println!("\nFinal HC families selected by size >= {} OR Pearson p <= {}: {}", args.min_family_size, args.max_pearson_p, selected.len());
    println!("  {:<72} {:>6}  {:>6}  {:>9}  {:>6}  {:>8}  {:>8}  {:>16}", "HC family", "cells", "HC max", "LC clones", "LC n", "LC mean", "LC SD", "LC top 3");
    println!("  {}", "-".repeat(145));
    for f in selected {
...
struct PearsonStat { n: usize, r: Option<f64>, p: Option<f64> }
...
fn pearson(x: &[f64], y: &[f64]) -> PearsonStat {
    ...
    if n < 3 { return PearsonStat { n, ..Default::default() }; }
    let (x, y) = (&x[..n], &y[..n]);
    ...
    if sxx == 0.0 || syy == 0.0 { return PearsonStat { n, ..Default::default() }; }
    let r = (sxy / (sxx * syy).sqrt()).clamp(-1.0, 1.0);
    let p = if r.abs() >= 1.0 { 0.0 } else {
        let t = r.abs() * (((n - 2) as f64) / (1.0 - r * r)).sqrt();
        let dist = StudentsT::new(0.0, 1.0, (n - 2) as f64).expect("valid Student t distribution");
        2.0 * (1.0 - dist.cdf(t))
    };
    PearsonStat { n, r: Some(r), p: Some(p) }
}

fn family_plot_analysis(family: &Family) -> FamilyPlotAnalysis {
    let hc_by_cell: HashMap<&str, (&str, usize)> = family.members.iter().map(|m| {
        (m.cell.cell_id.as_str(), (m.cell.hc.c.as_str(), m.hc_mutations.mutation_events()))
    }).collect();
    ...
    for lc in &family.light_clones {
        for member in &lc.members {
            let Some(&(isotype, hc_depth)) = hc_by_cell.get(member.cell.as_str()) else { continue };
            paired_cells.insert(member.cell.clone());
            let lc_depth = member.mutations.mutation_events();
            let state = states.entry((hc_depth, lc.name.clone(), Some(lc_depth))).or_default();
            state.cells.insert(member.cell.clone());
            *state.isotypes.entry(if isotype.is_empty() { "unknown" } else { isotype }.to_string()).or_default() += 1;
            state.lc_name = lc.name.clone(); state.hc_depth = hc_depth; state.lc_depth = Some(lc_depth);
        }
    }
    for member in &family.members {
        if paired_cells.contains(&member.cell.cell_id) { continue; }
        let hc_depth = member.hc_mutations.mutation_events();
        let state = states.entry((hc_depth, "unpaired".to_string(), None)).or_default();
        state.cells.insert(member.cell.cell_id.clone());
        let iso = if member.cell.hc.c.is_empty() { "unknown" } else { member.cell.hc.c.as_str() };
        *state.isotypes.entry(iso.to_string()).or_default() += 1;
        state.lc_name = "unpaired".to_string(); state.hc_depth = hc_depth;
    }
    ...
    let total: Vec<f64> = paired.iter().map(|s| (s.hc_depth + s.lc_depth.unwrap()) as f64).collect();

    let mut iso_cat = vec!["HC NAIVE".to_string()];
    let mut lc_cat = vec!["HC NAIVE".to_string()];
    let mut hd = vec![None]; let mut ld = vec![None]; let mut pd = vec![None];
    for s in &sv {
        iso_cat.push(s.isotypes.iter().max_by(|(ka,va),(kb,vb)| va.cmp(vb).then_with(|| kb.cmp(ka))).map(|(k,_)| k.clone()).unwrap_or_else(|| "unknown".into()));
        lc_cat.push(s.lc_name.clone());
        hd.push(Some(s.hc_depth as f32)); ld.push(s.lc_depth.map(|x| x as f32));
    ...
    lc_order.sort_by_key(|name| std::cmp::Reverse(family.light_clones.iter().find(|x| &x.name == name).map(|x| x.members.len()).unwrap_or(0)));
    lc_order.dedup(); if lc_cat.iter().any(|x| x == "unpaired") { lc_order.push("unpaired".into()); }
    let mut hclc = BTreeMap::new();
    for lc_name in family.light_clones.iter().map(|x| x.name.clone()) {
        let subset: Vec<&PlotState> = paired.iter().copied().filter(|s| s.lc_name == lc_name).collect();
        ...
        let total: Vec<f64> = subset.iter().map(|s| (s.hc_depth + s.lc_depth.unwrap()) as f64).collect();
        hclc.insert(lc_name, HclcAnalysis {
    ...
    let hd_colors = rooted_continuous_hex(&hd, 0); let ld_colors = rooted_continuous_hex(&ld, 0); let pd_colors = rooted_continuous_hex(&pd, 0);
    let encode = |colors: &[String]| -> String {
        sv.iter().enumerate().flat_map(|(i,s)| s.cells.iter().map(move |cell| format!("{}={}", cell, colors[i+1]))).collect::<Vec<_>>().join(";")
    };
    FamilyPlotAnalysis {
        abundance_hc: pearson(&abundance, &hc), abundance_lc: pearson(&abundance, &lc),
        abundance_paired: pearson(&abundance, &total), hc_lc: pearson(&hc, &lc),
        isotype_hex: encode(&iso_colors), light_chain_hex: encode(&lc_colors), hc_depth_hex: encode(&hd_colors),
        lc_depth_hex: encode(&ld_colors), paired_depth_hex: encode(&pd_colors), hclc,
    }
}
...
fn family_is_significant(a: &FamilyPlotAnalysis, max_p: f64) -> bool {
    stats_significant(&[a.abundance_hc, a.abundance_lc, a.abundance_paired, a.hc_lc], max_p)
        || a.hclc.values().any(|x| stats_significant(&[x.abundance_hc, x.abundance_lc, x.abundance_paired, x.hc_lc], max_p))
}
...
fn write_one_family_plot(out: &Path, family: &Family, plot_index: usize, _min_size: usize, _max_p: f64, radial_layout: bool) -> Result<()> {
    ...
    labels.extend(state_values.iter().enumerate().map(|(i, s)| {
        format!("state{}|HC{}|{}|LC{}", i + 1, s.hc_depth, s.lc_name, s.lc_depth.map(|x| x.to_string()).unwrap_or_else(|| "NA".into()))
    }));
    ...
    for s in &state_values {
        ...
        let dominant_iso = s.isotypes.iter().max_by(|(ka, va), (kb, vb)| va.cmp(vb).then_with(|| kb.cmp(ka)))
            .map(|(k, _)| k.clone()).unwrap_or_else(|| "unknown".to_string());
    ...
    model.tree.plot_rooted_annotated_cached(n, 0, &iso_cat, None, &iso_mixed, &abundance, "IGH constant class", &title, &path("mst_rooted_isotype.svg"), radial_layout)
        .map_err(|e| anyhow::anyhow!("isotype SVG failed for {}: {e}", family.name))?;
    model.tree.plot_rooted_annotated_cached(n, 0, &lc_cat, Some(&lc_order), &lc_mixed, &abundance, "Light-chain clone", &title, &path("mst_rooted_light_chain.svg"), radial_layout)
        .map_err(|e| anyhow::anyhow!("LC SVG failed for {}: {e}", family.name))?;
    model.tree.plot_rooted_continuous_cached(n, 0, &hc_depth, &abundance, "HC mutational depth (events)", &title, &path("mst_rooted_hc_depth.svg"), radial_layout)
        .map_err(|e| anyhow::anyhow!("HC-depth SVG failed for {}: {e}", family.name))?;
    model.tree.plot_rooted_continuous_cached(n, 0, &lc_depth, &abundance, "LC mutational depth (events)", &title, &path("mst_rooted_lc_depth.svg"), radial_layout)
        .map_err(|e| anyhow::anyhow!("LC-depth SVG failed for {}: {e}", family.name))?;
    model.tree.plot_rooted_continuous_cached(n, 0, &paired_depth, &abundance, "Paired HC+LC mutational depth (events)", &title, &path("mst_rooted_paired_depth.svg"), radial_layout)
        .map_err(|e| anyhow::anyhow!("paired-depth SVG failed for {}: {e}", family.name))?;
    Ok(())
}

fn receptor_complete(r: &Receptor) -> bool {
    !r.v.is_empty() && !r.j.is_empty() && !r.cdr3_nt.is_empty() && !r.naive.is_empty() && !r.observed.is_empty()
}

fn build_initial_families(mut cells: Vec<CellReceptor>, cfg: &FamilyConfig) -> Vec<Family> {
    cells.sort_by(|a,b| (&a.hc.v,&a.hc.j,&a.hc.cdr3_nt,&a.cell_id).cmp(&(&b.hc.v,&b.hc.j,&b.hc.cdr3_nt,&b.cell_id)));
...
fn reassign_cells(families: &mut [Family], rejected: Vec<(String, CellReceptor)>, cfg: &FamilyConfig, novel_v: &ReferenceModels) -> Vec<UnassignedCell> {
    ...
    for (from_family, rejected_cell) in rejected {
        ...
        for family in families.iter_mut() {
            if family.name == from_family { continue; }
        ...
        if let Some(cell) = cell { unassigned.push(UnassignedCell { cell, from_family }); }
    }
    unassigned
}
...
fn read_vdj_output(dir: &Path, source: &str) -> Result<Vec<CallRow>> {
    ...
    if !calls_path.is_file() { bail!("missing {}", calls_path.display()); }
    ...
    let req = |name: &str| -> Result<usize> {
        index.get(name).copied().with_context(|| format!("{}: missing required AIRR column {name}", airr_path.display()))
    };
    ...
    let j_i = req("j_call")?;
    let c_i = req("c_call")?;
    let cdr3_i = index.get("cdr3").copied().or_else(|| index.get("junction").copied())
        .with_context(|| format!("{}: missing cdr3/junction", airr_path.display()))?;
    let observed_i = req("sequence")?;
    let productive_i = req("productive")?;
    let max_i = [cell_i,id_i,chain_i,v_i,d_i,j_i,c_i,cdr3_i,observed_i,productive_i].into_iter().max().unwrap();
    ...
    for (line_no, line) in lines.enumerate() {
        ...
        if fields.len() <= max_i {
            bail!("{}:{} has {} fields, need at least {}", airr_path.display(), line_no + 2, fields.len(), max_i + 1);
        }
    ...
    if missing_naive > 0 {
        eprintln!("ClonoMap: skipped {missing_naive} AIRR rows without matching (cell, recombination_id) naive_recombination in vdj_calls.tsv");
    }
    Ok(out)
}
...
fn read_reconstructions(path: &Path) -> Result<HashMap<(String, String), SelectedReconstruction>> {
    ...
    let index: HashMap<&str, usize> = names.iter().enumerate().map(|(i,n)| (*n,i)).collect();
    let req = |name: &str| -> Result<usize> {
        index.get(name).copied().with_context(|| format!("{}: missing required column {name}", path.display()))
    };
    let cell_i = req("cell")?;
    ...
    let max_i = [cell_i,id_i,naive_i,rearr_i,receptor_i,productivity_i,support_i,junction_support_i,junction_spanning_i,rediscovery_i]
    ...
    for (line_no, line) in lines.enumerate() {
        ...
        let fields: Vec<&str> = line.split('\t').collect();
        if fields.len() <= max_i {
            bail!("{}:{} has {} fields, need at least {}", path.display(), line_no + 2, fields.len(), max_i + 1);
        }
        let cell = fields[cell_i].trim();
        let id = fields[id_i].trim();
        ...
        if cell.is_empty() || id.is_empty() || naive.is_empty() || observed_rearrangement.is_empty() || observed_receptor.is_empty() { continue; }
        let measurement = align_fragment(naive, observed_rearrangement);
        let mutation_events = measurement.as_ref().map_or(usize::MAX, |m| m.mutation_events());
        candidates.entry((cell.to_string(), id.to_string())).or_default().push(ReconstructionCandidate {
    ...
    for (key, mut rows) in candidates {
        ...
        let evidence_reads = best.receptor_rediscovery_reads.max(best.junction_spanning_reads).max(best.junction_support_reads);
        out.insert(key, SelectedReconstruction {
            naive: best.naive,
    ...
    eprintln!("ClonoMap receptor normalization: {ambiguous_keys} cell/recombination calls had alternative reconstructions; selected one and discarded {discarded_rows} alternatives");
    Ok(out)
}

fn reconstruction_rank(c: &ReconstructionCandidate) -> (bool, usize, std::cmp::Reverse<usize>, std::cmp::Reverse<usize>, std::cmp::Reverse<usize>, std::cmp::Reverse<usize>, std::cmp::Reverse<usize>, String, String) {
    let informative = align_fragment(&c.naive, &c.observed_rearrangement).map_or(0, |m| m.informative_pairs);
    (
        !c.productive,
        c.mutation_events,
...
fn parse_count(s: &str) -> usize { s.trim().parse().unwrap_or(0) }

fn productive_value(s:&str)->bool { matches!(s.trim().to_ascii_lowercase().as_str(),"productive"|"true"|"t"|"yes"|"1") }
...
fn write_family_report(path:&Path,families:&[Family])->Result<()> {
    ...
    for f in rows {
        ...
        writeln!(
            ...
            r.lc_clones,
            r.lc_mutations.n,
            r.lc_mutations.mean.map_or("NA".into(), |x| format!("{x:.4}")),
            r.lc_mutations.sd.map_or("NA".into(), |x| format!("{x:.4}")),
            max3.next().map_or("NA".into(), |x| x.to_string()),
            max3.next().map_or("NA".into(), |x| x.to_string()),
            max3.next().map_or("NA".into(), |x| x.to_string()),
            a.abundance_hc.n, fmt_stat(a.abundance_hc.r), fmt_stat(a.abundance_hc.p),
            a.abundance_lc.n, fmt_stat(a.abundance_lc.r), fmt_stat(a.abundance_lc.p),
            a.abundance_paired.n, fmt_stat(a.abundance_paired.r), fmt_stat(a.abundance_paired.p),
            a.hc_lc.n, fmt_stat(a.hc_lc.r), fmt_stat(a.hc_lc.p),
            a.isotype_hex, a.light_chain_hex, a.hc_depth_hex, a.lc_depth_hex, a.paired_depth_hex,
        )?;
    }
    Ok(())
}
...
fn fmt_stat(x: Option<f64>) -> String { x.map_or("NA".into(), |v| format!("{v:.6}")) }
