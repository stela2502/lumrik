...
use crate::{Receptor, align_fragment};
...
impl Default for ReferenceModels {
    fn default() -> Self {
        Self {
            curator: ReferenceCurator::new(),
            entries: Vec::new(),
            technical_detector: None,
        }
    }
}
...
impl ReferenceModels {
    pub fn open(path: Option<&Path>, chemistries: &[Chemistry]) -> Result<Self, String> {
        ...
        Ok(Self {
            curator,
            entries: Vec::new(),
            technical_detector: Some(detector),
        })
    }
    ...
    pub fn hypothesize_receptor_for_sample(
        &mut self,
        receptor: &mut Receptor,
        sample: Option<&str>,
    ) -> bool {
        if !matches!(receptor.chain.as_str(), "IGH" | "IGK" | "IGL") {
            return false;
        }
        let original_v = receptor.v.clone();
        let Some(candidate_v) = candidate_v_fragment(receptor, self.technical_detector.as_ref())
        else {
            return false;
        };
        ...
        if let Some(entry) = self.entries.iter_mut().find(|entry| {
            entry.chain == receptor.chain
                ...
                && self.curator.candidate(&entry.id).is_some_and(|candidate| {
                    ...
                    align_fragment(&String::from_utf8_lossy(model), &candidate_v)
                        .is_some_and(|m| m.identity >= 0.75)
                })
        }) {
        ...
        let Ok(id) = self.curator.observe(candidate_v.as_bytes(), observation) else {
            return false;
        };
    ...
    pub fn resolve_receptor_for_sample(
        &mut self,
        receptor: &mut Receptor,
        hard_identity: f64,
        sample: Option<&str>,
    ) -> bool {
        if receptor.evidence_reads < 2 {
            return false;
        }
        if !matches!(receptor.chain.as_str(), "IGH" | "IGK" | "IGL") {
            return false;
        ...
        let Some(reference_fit) = align_fragment(&receptor.naive, &candidate_v) else {
            return false;
        };
        if reference_fit.identity >= hard_identity {
            return false;
        }
    ...
    pub fn effective_naive(&self, receptor: &Receptor) -> String {
        let Some(candidate) = self.curator.candidate(&receptor.v) else {
            return receptor.naive.clone();
        };
        let model = candidate
            .resolved
            .as_ref()
        ...
        .unwrap_or_else(|| receptor.naive.clone())
    }

    pub fn total(&self) -> usize {
        self.entries.len()
    }
    pub fn entries(&self) -> &[ReferenceModelEntry] {
        &self.entries
    }
    pub fn curator(&self) -> &ReferenceCurator {
        &self.curator
    }
    pub fn save(&self, path: &Path) -> Result<(), String> {
        self.curator.save(path).map_err(|e| e.to_string())
    }
}
...
mod tests {
    ...
    fn unresolved_candidate_is_owned_by_reference_curator() {
        ...
        let mut receptor = Receptor {
            ...
            naive:
                "CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCTTTTGGGG"
                    .into(),
            observed:
                "GCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTGCTTTTGGGG"
                    .into(),
