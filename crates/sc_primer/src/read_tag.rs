...
impl ReadTagRecord {
    ...
    pub fn from_qname(qname: &str) -> Result<Self> {
        ...
        let next = fields
            .next()
            .ok_or_else(|| anyhow::anyhow!("QNAME metadata is not terminated: '{qname}'"))?;
    ...
    pub fn cell_string(&self) -> String {
        String::from_utf8_lossy(&self.cell_seq).into_owned()
    }
    pub fn umi_string(&self) -> String {
        String::from_utf8_lossy(&self.umi_seq).into_owned()
    }
