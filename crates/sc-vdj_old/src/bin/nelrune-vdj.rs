...
use sc_vdj::{
    read_bam_receptor_evidence_with_progress_and_batch_size, Chain, ExpressionMatrix, GermlineSegmentSupport,
...
struct Cli {
    ...
    /// Internal receptor-evidence staging threshold. Hidden because production
    /// runs should use the memory-bounded default; integration tests can lower
    /// it to force repeated compaction cycles on a tiny fixture.
    #[arg(long, default_value_t = sc_vdj::DEFAULT_ROUTED_EVIDENCE_BATCH_SIZE, hide = true)]
...
impl Cli {
    fn validate(mut self) -> Result<Self> {
        ...
        if self.routing_batch_size == 0 {
            bail!("--routing-batch-size must be greater than zero");
        }
    ...
    fn write_cells(&mut self, cells: &[sc_vdj::CellVdjSummary], reference: &VdjReference) -> Result<()> {
        for cell in cells {
            ...
            for call in &cell.rearrangements {
                ...
                let airr = sc_vdj::annotate_rearrangement(call);
                ...
                let np2 = junction
                    ...
                    .unwrap_or_default();
                let sequence_id = if rearrangement_id.is_empty() {
                    format!("{}|{}|{}", cell.cell, call.chain, call.notation)
                } else {
                    format!("{}|{}", cell.cell, rearrangement_id)
                };
                let airr_fields = vec![
                    sequence_id,
                    text(&airr.sequence),
                    ...
                    text(&airr.cdr3),
                    text(&airr.cdr3_aa),
                    (!airr.junction.is_empty()).then(|| airr.junction.len().to_string()).unwrap_or_default(),
                    (!airr.junction_aa.is_empty()).then(|| airr.junction_aa.len().to_string()).unwrap_or_default(),
                    airr.cdr3_start.map(|x| x.to_string()).unwrap_or_default(),
                    airr.cdr3_end.map(|x| x.to_string()).unwrap_or_default(),
                    junction.map(|_| np1.len().to_string()).unwrap_or_default(),
                    junction.filter(|_| call.chain.has_d()).map(|_| np2.len().to_string()).unwrap_or_default(),
                    cell.cell.clone(),
                    call.total_supporting_umis.to_string(),
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "nelrune-vdj: {} cells, batched receptor-evidence routing, {} reference segments",
        gex.cells.len(),
        reference.len()
    );
    ...
    let (routed, stats) = read_bam_receptor_evidence_with_progress_and_batch_size(
        &cli.bam,
        &NelruneIdentityResolver,
        |cell| called_cells.contains(cell),
        reference,
        cli.routing_batch_size,
        move |stats| {
    ...
    mapping_info.report_n("vdj.routing_batch_flushes", stats.batch_flushes);
    mapping_info.report_n("vdj.compacted_sequences", stats.compacted_sequences);
    mapping_info.report_n("vdj.redundant_segments", stats.redundant_segments);
    ...
    eprintln!(
        "nelrune-vdj: BAM routed: total={} receptor_chr={} segment_overlap={} called_cell={} routed={} batches={} compacted={} redundant={}",
        stats.total_records,
        ...
        stats.batch_flushes,
        stats.compacted_sequences,
        stats.redundant_segments
    );
