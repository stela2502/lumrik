...
impl OneHotGenome {
    fn push(&mut self, seq: &[u8]) -> usize {
        ...
        self.reverse
            .extend(seq.iter().rev().copied().map(complement_one_hot));
        self.spans.push(ReferenceSpan {
            forward_start,
            reverse_start,
            len: seq.len() as u32,
        });
...
impl FastTagMapper {
    pub fn new() -> Self {
        Self {
            ...
            features: Vec::new(),
            genome: OneHotGenome::default(),
            packed_features: Vec::new(),
            packed_reverse_features: Vec::new(),
            min_hits: 4,
        }
    }
    ...
    pub fn add_feature(&mut self, seq: &[u8], feature: FeatureEntry) -> usize {
        ...
        assert!(
            ...
            "too many fast-mapper features"
        );
        self.features.push(feature);
        self.genome.push(seq);
        self.packed_features.push(OneHotSequence::from_bytes(seq));
        self.packed_reverse_features
            .push(OneHotSequence::from_bytes(&reverse_complement(seq)));
    ...
    fn index_orientation(&mut self, seq: &[u8], feature_index: usize, reverse: bool) {
        for (ref_pos, seed) in Rolling16::new(seq) {
            ...
            if bin.iter().any(|e| {
                e.prefix_key == prefix_key
                    && e.confirm == confirm
                    && e.feature_index == feature_index as u32
                    && e.ref_pos == ref_pos
                    && e.reverse == reverse
            }) {
                continue;
    ...
    pub fn map_feature_id_with_qual(
        &self,
        seq: &[u8],
        qual: &[u8],
        mapping: &mut MappingInfo,
    ) -> Option<u64> {
        match self.map_status_with_qual(seq, qual, mapping) {
            MapStatus::Hit { feature_id, .. } => Some(feature_id),
            _ => None,
        }
    }
    ...
    pub fn align(&self, seq: &[u8], qual: Option<&[u8]>) -> Option<FastAlignment> {
        ...
        if best.next().is_some() || winner.candidate.hits < self.min_hits {
            return None;
        }
        let second_best_score = verified
            .iter()
            .filter(|x| {
                x.candidate.feature_index != winner.candidate.feature_index
                    || x.candidate.query_start != winner.candidate.query_start
                    || x.candidate.reverse != winner.candidate.reverse
            })
            .map(|x| x.score)
            .max();
    ...
    fn discover_candidates(&self, seq: &[u8]) -> Vec<Candidate> {
        ...
        for (query_pos, seed) in Rolling16::new(seq) {
            ...
            for entry in &self.bins[bin_index] {
                ...
                let query_start = query_pos as isize - entry.ref_pos as isize;
                if let Some(c) = candidates.iter_mut().find(|c| {
                    c.feature_index == entry.feature_index as usize
                        && c.query_start == query_start
                        && c.reverse == entry.reverse
                }) {
                    c.hits += 1;
                } else {
                    candidates.push(Candidate {
                        feature_index: entry.feature_index as usize,
                        query_start,
    ...
    fn verify_candidate(
        ...
        let reference = self
            .genome
            .slice(candidate.feature_index, candidate.reverse);
    ...
    fn resolve_verified(&self, verified: &[VerifiedCandidate]) -> MapStatus {
        let Some(best_score) = verified.iter().map(|x| x.score).max() else {
        ...
        if best.len() != 1 {
            ...
            let mut feature_ids: Vec<_> = best
                .iter()
                .map(|x| self.features[x.candidate.feature_index].id)
                .collect();
        ...
        MapStatus::Hit {
            feature_id: self.features[c.feature_index].id,
...
#[inline]
fn match_reward(q: u8) -> i32 {
    2 + i32::from(q) / 20
}
#[inline]
fn mismatch_penalty(q: u8) -> i32 {
    2 + i32::from(q) / 5
}

#[inline]
fn one_hot(base: u8) -> u8 {
    match base {
        b'A' | b'a' => 1,
        b'C' | b'c' => 2,
        b'G' | b'g' => 4,
...
#[inline]
fn complement_one_hot(base: u8) -> u8 {
    match base {
        b'A' | b'a' => 8,
        b'C' | b'c' => 4,
        b'G' | b'g' => 2,
...
impl Iterator for Rolling16<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.pos < self.seq.len() {
            ...
            let bits = match base {
                b'N' | b'n' => None,
                _ => IntToDna::encode_binary(base).ok().map(u32::from),
            };
            match bits {
                Some(bits) => {
                    ...
                    if self.valid_bases >= SEED_BASES {
                        return Some((self.pos - SEED_BASES, self.word));
                    }
                }
                None => {
                    self.word = 0;
...
pub(crate) fn encode_8mer(seq: &[u8]) -> Option<u16> {
    ...
    for &base in seq {
        ...
        word = (word << 2) | u16::from(IntToDna::encode_binary(base).ok()?);
    }
    Some(word)
}
...
mod tests {
    ...
    #[test]
    fn one_hot_reference_has_both_strands() {
        let mut g = OneHotGenome::default();
        g.push(b"ACGTAA");
        assert_eq!(g.slice(0, false), &[1, 2, 4, 8, 1, 1]);
        assert_eq!(g.slice(0, true), &[8, 8, 1, 2, 4, 8]);
    }
    #[test]
    fn quality_changes_mismatch_score() {
        assert!(mismatch_penalty(40) > mismatch_penalty(5));
    }
}
