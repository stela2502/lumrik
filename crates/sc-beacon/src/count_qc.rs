...
fn write_log_umi_distribution<P: AsRef<Path>>(
    ...
    let logs: Vec<f64> = counts
        .iter().copied().filter(|&n| n > 0)
        .map(|n| (n as f64).log10()).collect();
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .margin(20).x_label_area_size(55).y_label_area_size(75)
        .build_cartesian_2d(0.0..max_x, 0.0..(max_y * 1.08))?;
    chart.configure_mesh()
        .x_desc("log10(exonic UMIs per barcode)").y_desc("Barcodes").draw()?;
    chart.draw_series(bins.iter().enumerate().map(|(i, &n)| {
        let x0 = i as f64 * bin_width;
        Rectangle::new([(x0, 0.0), (x0 + bin_width, n as f64)], BLUE.mix(0.35).filled())
    }))?;
    chart.draw_series(std::iter::once(PathElement::new(
        vec![(cutoff_x, 0.0), (cutoff_x, max_y * 1.05)], RED.stroke_width(2),
    )))?;
    chart.draw_series(std::iter::once(Text::new(
        format!("knee cutoff: {} UMIs", fit.umi_cutoff),
        (cutoff_x, max_y * 1.02), ("sans-serif", 18).into_font(),
    )))?;
    root.present().with_context(|| format!("writing {}", path.as_ref().display()))?;
    Ok(())
}
...
fn write_rank_plot<P: AsRef<Path>>(counts: &[u32], fit: &KneeCountFit, path: P) -> Result<()> {
    ...
    chart.configure_mesh().x_desc("log10(barcode rank)").y_desc("log10(exonic UMIs)").draw()?;
    chart.draw_series(LineSeries::new(
        sorted.iter().enumerate().map(|(i, &n)| (((i + 1) as f64).log10(), (n as f64).log10())),
        &BLUE,
    ))?;
    chart.draw_series(std::iter::once(PathElement::new(
        vec![(knee_x, 0.0), (knee_x, max_count * 1.03)], RED.stroke_width(2),
    )))?;
    chart.draw_series(std::iter::once(PathElement::new(
        vec![(0.0, knee_y), (max_rank, knee_y)], RED.mix(0.55).stroke_width(1),
    )))?;
    chart.draw_series(std::iter::once(Circle::new((knee_x, knee_y), 5, RED.filled())))?;
    chart.draw_series(std::iter::once(Text::new(
        format!("knee: rank {}, {} UMIs", fit.knee_rank, fit.umi_cutoff),
        (knee_x, (knee_y + 0.08).min(max_count)), ("sans-serif", 18).into_font(),
    )))?;
    root.present().with_context(|| format!("writing {}", path.as_ref().display()))?;
    Ok(())
}
