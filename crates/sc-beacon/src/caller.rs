...
impl GuideCalls {
    pub fn from_model(model: &FittedModel, cfg: &CallConfig) -> Self {
        ...
        for ((state, p_value), q_value) in model.observations.iter().zip(pvalues).zip(qvalues) {
            let obs = state.observation;
            ...
            let posterior = state.evidence;

            let called =
                posterior.probability >= cfg.minimum_posterior && q_value <= cfg.maximum_fdr;
        ...
        for calls in by_cell.values_mut() {
            calls.sort_unstable_by_key(|call| call.guide_id);
        }
