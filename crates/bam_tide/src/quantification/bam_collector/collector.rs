...
impl BamCollector {
    ...
    pub fn from_cli(
        ...
        let match_options = MatchOptions {
            require_strand:
                config.require_strand,

            require_exact_junction_chain:
                config.require_exact_junction_chain,

            max_5p_overhang_bp:
                config.max_5p_overhang_bp,

            max_3p_overhang_bp:
                config.max_3p_overhang_bp,

            allowed_intronic_gap_size:
                config.allowed_intronic_gap_size,
        };
    ...
    pub fn spawn(
        self,
        stdout: ChildStdout,
    ) -> Result<BamCollectorHandle> {
        let handle = thread::spawn(move || {
            let reader =
                Self::reader_from_stdout(stdout)?;

            self.collect(reader)
        });

        Ok(BamCollectorHandle {
            handle,
        })
    }
    ...
    fn collect_reader(
        ...
        let read_tag =
            ...
            .with_context(|| {
                format!(
                    "failed to recover ReadTagRecord from QNAME '{qname}'"
                )
            })?;

            record.set_qname(
                read_tag.read_id.as_bytes()
            );
    ...
    fn flush_jobs(
        ...
        processor.process_into(
            self.config.quant_mode,
            jobs,
            data,
        )?;

        data.report
            .stop_single_processor_time();
    ...
    fn reader_from_stdout(
        ...
        .context(
            "opening mapper stdout as SAM/BAM"
        )
    }
}
