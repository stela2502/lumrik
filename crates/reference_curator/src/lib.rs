...
impl ReferenceCurator {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let magic: [u8; 8] =
            bincode::deserialize_from(&mut reader).context("read reference-curator magic")?;
        if &magic != STORE_MAGIC {
            bail!("{} is not a reference_curator store", path.display());
        }
        let store: Self =
            bincode::deserialize_from(&mut reader).context("read reference-curator store")?;
        if store.version != STORE_VERSION {
            bail!(
                "unsupported reference_curator store version {} (expected {})",
                store.version,
                STORE_VERSION
            );
        }
    ...
    pub fn save(&self, path: impl AsRef<Path>) -> Result<()> {
        ...
        let tmp = temporary_path(path);
        {
            ...
            bincode::serialize_into(&mut writer, STORE_MAGIC)
                .context("write reference-curator magic")?;
    ...
    pub fn observe(
        ...
        if let Some(id) = self.sequence_index.get(&sequence).cloned() {
            self.candidates
                .get_mut(&id)
                .expect("sequence index must point to candidate")
                .observations
                .push(observation);
            return Ok(id);
        }
    ...
    pub fn add_annotation(
        ...
        if reference.trim().is_empty() {
            bail!("reference id must not be empty");
        }
    ...
    pub fn resolve(&mut self, candidate_id: &str, resolved: ResolvedReference) -> Result<()> {
        ...
        for evidence_id in &resolved.evidence_ids {
            if !candidate
                ...
                .values()
                .flatten()
                .any(|a| a.evidence_id == *evidence_id)
            {
    ...
    pub fn candidate(&self, id: &str) -> Option<&ReferenceCandidate> {
        self.candidates.get(id)
    }
    pub fn candidates(&self) -> impl Iterator<Item = &ReferenceCandidate> {
    ...
    pub fn unresolved(&self) -> impl Iterator<Item = &ReferenceCandidate> {
        self.candidates.values().filter(|c| c.resolved.is_none())
    }
    pub fn resolved(&self) -> impl Iterator<Item = &ReferenceCandidate> {
        self.candidates.values().filter(|c| c.resolved.is_some())
    }
    ...
    pub fn export_fasta(&self, path: impl AsRef<Path>, filter: CandidateFilter) -> Result<()> {
        ...
        for candidate in candidates {
            let sequence = match (&candidate.resolved, filter) {
                (Some(resolved), CandidateFilter::All | CandidateFilter::ResolvedOnly) => {
                    &resolved.sequence
                }
                (None, CandidateFilter::All | CandidateFilter::UnresolvedOnly) => {
                    &candidate.sequence
                }
                (Some(_), CandidateFilter::UnresolvedOnly)
                | (None, CandidateFilter::ResolvedOnly) => continue,
            };
    ...
    fn validate(&self) -> Result<()> {
        if self.version != STORE_VERSION {
            bail!("invalid store version {}", self.version);
        }
        if self.sequence_index.len() != self.candidates.len() {
            bail!("sequence index/candidate count mismatch");
        }
        for (sequence, id) in &self.sequence_index {
            let candidate = self
                .candidates
                .get(id)
                .with_context(|| format!("sequence index points to missing candidate {id}"))?;
            if &candidate.sequence != sequence {
                bail!("sequence index mismatch for candidate {id}");
            }
        }
        Ok(())
    }
...
fn normalize_sequence(sequence: &[u8]) -> Result<Vec<u8>> {
    ...
    for &base in sequence {
        ...
        if !matches!(base, b'A' | b'C' | b'G' | b'T' | b'N') {
            bail!("unsupported sequence base {:?}", base as char);
        }
        out.push(base);
    }
    if out.is_empty() {
        bail!("candidate sequence must not be empty");
    }
    Ok(out)
}
...
mod tests {
    ...
    fn fasta_export_can_select_unresolved_candidates() {
        ...
        let path =
            std::env::temp_dir().join(format!("reference-curator-fasta-{}.fa", std::process::id()));
        curator
            .export_fasta(&path, CandidateFilter::UnresolvedOnly)
            .unwrap();
    ...
    fn unresolved_is_a_valid_persistent_state() {
        ...
        let path =
            std::env::temp_dir().join(format!("reference-curator-{}.bin", std::process::id()));
